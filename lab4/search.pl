...
df_path([Goal|Branch], Goal, [Goal|Branch]).
df_path([Leaf|Branch], Goal, Path) :-
  child(Leaf, Next),    /* For all children... */
  dif(Leaf, Goal),      /* Dont continue if goal is reached */
  not_in(Leaf, Branch), /* Prevent loops */
  df_path([Next,Leaf|Branch], Goal, Path).
